...
class TestDetectCarouselFraud:
    ...
    def test_multiple_cycles_returned(self):
        cycle_a = {
            "scc_id": 1, "cycle_ids": ["A", "B", "C"],
            "cycle_names": ["A Srl", "B Srl", "C Srl"],
            "cycle_length": 3, "cigs": [],
        }
        cycle_b = {
            "scc_id": 2, "cycle_ids": ["D", "E", "F", "G"],
...
class TestDetectBoardOverlapCollusion:
    ...
    def test_multiple_pairs_returned(self):
        row_template = {
            "buyer_id": "b1", "buyer_name": "Ente", "shared_names": ["X"],
        }
        pairs = [
            {**row_template,
             "company1_id": f"c{i}", "company1_name": f"Azienda {i} SpA",
             "company2_id": f"c{i+1}", "company2_name": f"Azienda {i+1} Srl",
             "shared_count": 3}
            for i in range(3)
        ]
...
class TestDetectSubcontractorConcentration:
    ...
    def test_high_severity_risk_bump_called(self):
        ...
        conc_row = {
            "winner_id": "w1", "winner_name": "W", "sub_id": "s1",
            "sub_name": "S", "pair_count": 5, "concentration_ratio": 0.9,
            "cigs": [],
        }
...
class TestRunAllDetectorsExtended:
    ...
    def test_registry_contains_13_detectors(self):
        ...
        assert len(called) == 13, (
            f"Expected 13 detectors, got {len(called)}: {called}"
        )
