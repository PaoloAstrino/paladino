...
class MockRecord(dict):
    ...
    def __init__(self, data=None):
        processed_data = {
            k: (MockRecord(v) if isinstance(v, dict) else v)
    ...
    def __getitem__(self, key):
        ...
        if "nome" in str(key).lower(): return "Milano"
        if "titolo" in str(key).lower(): return "Test Project"
        if "oggetto" in str(key).lower(): return "Test tender"
        if "status" in str(key).lower(): return "Completato"
        if "price" in str(key).lower(): return 120000.0
        if "amount" in str(key).lower(): return 1200000.0
        if "sim" in str(key).lower(): return 0.95
        if "conf" in str(key).lower(): return 0.95
        if "tenders" in str(key).lower(): return 10
        if "path_length" in str(key).lower(): return 2
        
        # Additional keys for integration tests
        if key in ['t', 'p', 'm', 'n', 'c', 'version', 'rel', 'path', 'current']:
            return MockRecord({"id": "test-id", "name": "Test"})
        if "created" in str(key).lower(): return True
        if "method" in str(key).lower(): return "test_method"
        if "old_price" in str(key).lower(): return 100000.0
        if "old_amount" in str(key).lower(): return 1000000.0
...
class MockResult:
    ...
    def list(self): return self._data
    def consume(self): return MagicMock()
...
def mock_driver():
    ...
    def _stateful_run(query, parameters=None, **kwargs):
        ...
        if "show constraints" in lower_query: return MockResult([{"name": "c1"}])
        if "show indexes" in lower_query: return MockResult([{"name": "i1"}])
        ...
        if "return" in lower_query and "count(" not in lower_query:
            if rows:
                ...
                return MockResult([{
                    ...
                    "risk_score": 0.8, "count": 1, "loaded": 1, "updated": 1,
                    "nome_normalizzato": "TEST COMPANY", "municipality": "Milano", "region": "Lombardia"
                }])
...
@pytest.fixture
def clean_neo4j(mock_driver): return mock_driver

@pytest.fixture
def mock_ollama():
    mock = MagicMock()
    mock.chat.return_value = json.dumps({
        "template": "high_risk_companies", "template_name": "high_risk_companies",
        "params": {"min_risk": 0.5}, "is_same": True, "confidence": 0.95
    })
...
def sample_ocds_tender():
    return {
        "ocid": "ocds-test-123",
        "tender": {
            "id": "CIG123", "title": "Test Tender", "description": "Test",
            "value": {"amount": 100000.0}, "procurementMethod": "open"
        },
        "awards": [{"id": "A1", "date": "2026-01-01T00:00:00Z", "suppliers": [{"id": "CF123", "name": "TEST COMPANY"}]}]
    }
