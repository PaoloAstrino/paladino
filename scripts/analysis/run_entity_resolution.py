    ...
    try:
        ...
        if duplicates_df.is_empty():
            ...
            for row in duplicates_df.head(5).iter_rows(named=True):
                logger.info(f"  {row['company_id_1']} ↔ {row['company_id_2']} "
                          f"(confidence: {row['confidence']}, method: {row['match_method']})")
        
        # Step 3: Merge duplicates
        logger.info("\n[3/4] Merging duplicates...")
        
        companies_df, same_as_df = deduplicator.merge_duplicates(
            companies_df,
            duplicates_df
        )
        ...
        logger.success(f"  - All companies enriched with statistics")
        ...
        with driver.session() as session:
            ...
            for record in result:
                logger.info(f"  {record['nome']}: risk={record['risk_score']:.2f}, "
