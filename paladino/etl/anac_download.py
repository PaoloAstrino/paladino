...
def _download_with_retry(session: requests.Session, url: str, timeout: int = 60) -> requests.Response:
    """
        ...
        try:
            ...
            if response.status_code == 429:
                ...
                if attempt < _MAX_RETRIES:
                    logger.warning(f"Rate-limited (429) on {url}. Waiting {retry_after}s (attempt {attempt}/{_MAX_RETRIES})…")
            ...
            if response.status_code >= 500:
                ...
                if attempt < _MAX_RETRIES:
                    logger.warning(f"Server error {response.status_code} for {url}. Retrying in {wait}s (attempt {attempt}/{_MAX_RETRIES})…")
                    time.sleep(wait)
    ...
    raise DownloadError(message=f"Download failed after {_MAX_RETRIES} retries: {url}") from last_exc
...
class AnacOcdsDownloader:
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        ...
        self.session.headers.update({
            'User-Agent': 'Paladino/0.1.0 (Italian Knowledge Graph)'
        })
    
    def fetch_release(self, year: int, month: int) -> Optional[Path]:
        """
        ...
        try:
            ...
            logger.success(f"Downloaded {cache_path.name} ({cache_path.stat().st_size / 1024 / 1024:.1f} MB)")
            return cache_path
