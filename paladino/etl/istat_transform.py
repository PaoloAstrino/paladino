...
class IstatTransformer:
    ...
    def transform_regions(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        try:
            ...
            reg_name_col = self._find_column(
                df, ["Denominazione Regione", "Denominazione regione", "DENOMINAZIONE"]
            )
        except IndexError as e:
            logger.error(f"Column mapping failed: {e}")
            raise e
        ...
        unique_regions = df.select(
            [pl.col(reg_code_col).alias("cod_regione"), pl.col(reg_name_col).alias("nome")]
