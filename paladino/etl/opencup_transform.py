...
class OpencupTransformer:
    ...
    def extract_projects(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if "fondi_comunitari" in available_cols:
            ...
            cols.append(pl.col("fondi_comunitari").fill_null("").map_elements(
                lambda x: [f.strip() for f in str(x).split(",") if f.strip()] if isinstance(x, str) else (x if isinstance(x, list) else []),
