...
class IstatDownloader:
    ...
    UNIFIED_CSV_URL = "https://www.istat.it/storage/codici-unita-amministrative/Elenco-comuni-italiani.csv"
    ...
    def fetch_unified_csv(self) -> Optional[Path]:
        """
        ...
        try:
            ...
            logger.success(f"Downloaded {cache_path.name} ({cache_path.stat().st_size / 1024:.1f} KB)")
            return cache_path
    ...
    def fetch_municipalities_csv(self) -> Optional[Path]:
        """Backward compatibility for municipalities CSV."""
        return self.fetch_unified_csv()
    
    def fetch_provinces_csv(self) -> Optional[Path]:
        """Backward compatibility for provinces CSV."""
        return self.fetch_unified_csv()
    
    def fetch_regions_csv(self) -> Optional[Path]:
