...
async def verify_api_key(
    ...
    valid_keys = settings.api_keys.split(",") if hasattr(settings, 'api_keys') and settings.api_keys else []
...
def get_rate_limit_key(request: Request, api_key: Optional[str] = None) -> str:
...
async def rate_limit_middleware(request: Request, call_next: Callable) -> Response:
    ...
    is_authenticated = bool(api_key and api_key in (settings.api_keys.split(",") if hasattr(settings, 'api_keys') and settings.api_keys else []))
...
class QueryAuditor:
    ...
    def _get_user_id(self, api_key: Optional[str]) -> Optional[str]:
