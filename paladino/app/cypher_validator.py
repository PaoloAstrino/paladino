...
class CypherValidator:
    ...
    WARNING_PATTERNS = [
        ...
        (
            r"\bMATCH\s+\([^)]*\),\s*\([^)]*\)",
            "Multiple MATCH without relationship - cartesian product risk",
        ),
    ...
    INJECTION_PATTERNS = [
        ...
        (
            ...
            "Parameter concatenation detected - potential injection",
        ),
    ]
    ...
    def validate(self, cypher: str) -> ValidationResult:
        ...
        if "$" not in cypher and self._has_user_input_markers(cypher):
            warnings.append(
                "Warning: Query has no parameters - ensure user input is properly escaped"
            )
